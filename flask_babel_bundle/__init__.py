...
from flask_babelex import Domain, lazy_gettext as _gettext, ngettext as _ngettext
...
class FlaskBabelBundle(Bundle):
    ...
    def before_init_app(cls, app: Flask):
        babel.localeselector(cls.get_locale)
        if cls.enable_url_lang_code_prefix:
...
def _get_domain(match):
    ...
    return Domain(
        pkg_resources.resource_filename(domain_name, 'translations'),
        domain=domain_name)
